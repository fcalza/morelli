...
//        jogador1.setDaVez(true);
     ...
     * @return
     */
    public boolean destinoMaisProximoAoCentro(Posicao origem, Posicao destino) {
                                    ...
                                    if (destino.equals(adjacente)) {
                                        ...
                                        if (oposta != null) {
                                            if (oposta.getCor() == cor) {
                                                posicao.setCor(cor);
                                                atualizaPosicaoTabuleiro(posicao);
                                            }
                                        }
                                    }
                                }
     ...
     * @param faixa
     */
    public boolean calcularQuadro(Posicao posicao, Faixa faixa) {
        // TODO - implement Tabuleiro.calcularQuadro
                        ...
                        if (vertice.getCor() == cor) {
                            ...
                            if (vertice.isOcupada()) {
                                if (vertice.getCor() == cor) {
                                    ...
                                    atualizaPosicaoTabuleiro(tronoTomado);
                                }
                            }
                        }
                    }
...
//        if (posicoes[indice + lado].isOcupada()) {
//
//        }
    ...
    public boolean calcularMovimento(Posicao origem, Posicao destino) {
        ...
        if (destino.isOcupada()) {
            ...
            boolean movimentoAoCentro = destinoMaisProximoAoCentro(origem, destino);
            if (!movimentoAoCentro) {
                atorJogador.notificarIrregularidade();

            } //Verifica se a peca se move na mesma linha
            else if (calcularMovimentoLinha(origem, destino)) {
                moverPeça(origem, destino);
                return true;
     ...
     * @param oposta
     */
    public void verificarAlinhamento(Posicao origem, Posicao adjacente, Posicao oposta) {
        // TODO - implement Tabuleiro.verificarAlinhamento
        throw new UnsupportedOperationException();
    }

    private void moverPeça(Posicao origem, Posicao destino) {
        destino.posicionarPeca(origem.getCor());
        origem.retirarPeca();
        atualizaPosicaoTabuleiro(origem);
        atualizaPosicaoTabuleiro(destino);
    }

    private void atualizaPosicaoTabuleiro(Posicao posicao) {
