...
public class Faixa implements Jogada {
    ...
    public Posicao getPosicaoCoordenada(int linha, int coluna) throws Exception {
        int i;
        for (i = 0; i < posicoes.length; i++) {
            if (posicoes[i].getLinha() == linha) {
                if (posicoes[i].getColuna() == coluna) {
                    break;
                }
            }
        }
     ...
     * @param indice
     */
    private void addPosicao(Posicao posicao, int indice) {
